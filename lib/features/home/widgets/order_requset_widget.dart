...
class OrderRequestWidget extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final order = deliveryRequestModel.order;
    ...
    if (order != null) {
      distance = Get.find<AddressController>().getRestaurantDistance(
        LatLng(
          double.parse(parcel ? order.deliveryAddress?.latitude ?? '0' : order.module?.moduleType == 'food' ? '0' : '0'),
          double.parse(parcel ? order.deliveryAddress?.longitude ?? '0' : order.module?.moduleType == 'food' ? '0' : '0'),
        ),
      );
    }
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      Expanded(
                        child: Column(
                          ...
                          children: [
                            Text(
                              parcel
                                  ? order?.module?.moduleName ?? ''
                                  : order?.module?.moduleName ?? 'no_store_data_found'.tr,
                              maxLines: 1,
                            ...
                            //   parcel
                            //       ? 'parcel'.tr
                            //       // : '${order?.detailsCount ?? 0} ${order?.detailsCount != null && order!.detailsCount! > 1 ? 'items'.tr : 'item'.tr}',
                      ...
                      Column(
                        ...
                        children: [
                          Text(
                            '${DateConverterHelper.timeDistanceInMin(order?.createdAt ?? '')} ${'mins_ago'.tr}',
                            style: PoppinsMedium.copyWith(
                              color: Theme.of(context).primaryColor,
                              fontSize: Dimensions.fontSizeSmall,
                            ),
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        Container(
                          ...
                          child: Text(
                            '${'payment'.tr} - ${order?.paymentMethod == 'cash_on_delivery' ? 'cod'.tr : order?.paymentMethod == 'wallet' ? 'wallet'.tr : 'digitally_paid'.tr}',
                            style: PoppinsMedium.copyWith(
                              fontSize: Dimensions.fontSizeExtraSmall,
                              color: Theme.of(context).disabledColor,
                            ),
                    ...
                    child: Row(
                      children: [
                        Expanded(
                          child: TextButton(
                            onPressed: () {
                              orderController.declineOrder(deliveryRequestModel.orderId,deliveryRequestModel.id);
                            }
                            // Get.dialog(
                          ...
                          child: CustomButtonWidget(
                            ...
                            onPressed: () => Get.dialog(
                              ConfirmationDialogWidget(
                                ...
                                onYesPressed: () {
                                  orderController
                                      .acceptOrder(deliveryRequestModel.orderId,deliveryRequestModel.id )
                                      .then((isSuccess) {
                                    if (isSuccess) {
                                      onTap();
                                      if (order != null) {
